...
class TestBackgroundAgent:
    ...
    async def test_spawn_returns_immediately(self):
        """spawn() should return a task ID without waiting."""
        import asyncio
        import time
        from banana.agent.subagent import SubagentManager
        ...
        class MockProvider:
            async def chat_stream_with_retry(self, **kw):
                ...
                from banana.providers.base import LLMResponse
                return LLMResponse(content="background result")
        ...
        task_id = await mgr.spawn(prompt="test", agent_type="general-purpose", timeout=10)
        elapsed = time.monotonic() - start

        assert elapsed < 0.3, f"spawn() took {elapsed:.2f}s, expected <0.3s"
        assert len(task_id) == 8

        await asyncio.sleep(1.0)
        completed = mgr.collect_completed()
        assert len(completed) == 1
        tid, result = completed[0]
        assert tid == task_id
        assert "background result" in result
        mgr.cancel_all_background()

    @pytest.mark.asyncio
    async def test_agent_tool_background(self):
        """AgentTool with background=True should use spawn()."""
        from banana.agent.subagent import SubagentManager

        class MockProvider:
            async def chat_stream_with_retry(self, **kw):
                from banana.providers.base import LLMResponse
                return LLMResponse(content="bg result")
        ...
        result = await t.execute(prompt="test", subagent_type="general-purpose", background=True)
        assert "Background agent launched" in result
        assert "ID:" in result
